    ...
    class ItemsDialog extends JDialog {
        ...
        class ItemsPanel extends JPanel implements MouseListener {
            ...
            public ItemsPanel(int sourceTree, int sourceNodeIndex) {
                ...
                for (int i = 0; i < n; i++) {
                    // Se mantiene la llamada con "/" y se deja que loadImage maneje la ruta
            ...
            // Método loadImage modificado para intentar cargar la imagen desde la ruta original
            // y, en caso de no encontrarla, agregar el prefijo "/OBJETOS/"
            private Image loadImage(String path) {
                URL url = getClass().getResource(path);
                if (url == null) {
                    // Si no se encuentra, se intenta con "/OBJETOS" + path
                    if (!path.startsWith("/")) {
                        path = "/" + path;
                    }
                    url = getClass().getResource("/OBJETOS" + path);
                    if (url == null) {
